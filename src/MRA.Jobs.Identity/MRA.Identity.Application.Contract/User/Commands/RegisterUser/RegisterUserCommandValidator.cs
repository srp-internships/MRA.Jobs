...
public class RegisterUserCommandValidator : AbstractValidator<RegisterUserCommand>
{
    public RegisterUserCommandValidator()
    {
        ...
        RuleFor(s => s.Password).Matches(@"^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$").WithMessage("Password must have at least one alphabetical character, one digit, one special character (@$!%*?&) and be at least 8 characters long.");

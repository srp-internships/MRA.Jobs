...
public class AddTagToApplicantCommandHandler : IRequestHandler<AddTagsToApplicantCommand, bool>
{
    ...
    public AddTagToApplicantCommandHandler(IApplicationDbContext context, IMapper mapper, IDateTime dateTime,
    ...
    public async Task<bool> Handle(AddTagsToApplicantCommand request, CancellationToken cancellationToken)
    {
        ...
        foreach (string tagName in request.Tags)
        {
            ...
            UserTag applicantTag = applicant.Tags.FirstOrDefault(t => t.Tag.Name == tagName);

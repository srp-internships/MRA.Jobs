    ...
    : IidentityService
{
    public async Task<UserProfileResponse> ApplicantDetailsInfo(string userName = null)
    {
        ...
        identityHttpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer",
            httpContextAccessor.HttpContext?.Request.Headers.Authorization[0]!.Split(' ')[1]);
        using var response = await identityHttpClient.GetAsync(
            $"{configuration["MraJobs-IdentityApi:Profile"]}{(userName != null ? $"?userName={userName}" : "")}");

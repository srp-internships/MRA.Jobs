...
public class TokenService
{
    ...
    {
        ApplicationUser user = _identityContext.Users.Find(userName);
        if (user != null)
        {
            string ipAddress = _httpContextAccessor.HttpContext.Connection.RemoteIpAddress.ToString();
            IEnumerable<RefreshToken> tokens = user.RefreshTokens.Where(t =>
                string.IsNullOrEmpty(t.RevokedByIp) && t.ExpiryOn > DateTime.UtcNow && t.CreatedByIp == ipAddress);

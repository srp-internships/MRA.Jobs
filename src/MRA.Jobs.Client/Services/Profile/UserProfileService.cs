...
public class UserProfileService(IdentityHttpClient identityHttpClient) : IUserProfileService
{
    ...
    {
        var result = await identityHttpClient
    ...
    {
        var result = await identityHttpClient.GetFromJsonAsync<List<UserEducationResponse>>("Profile/GetEducationsByUser");
        return result;
    }
    ...
    {
        var result = await identityHttpClient.GetFromJsonAsync<List<UserExperienceResponse>>("Profile/GetExperiencesByUser");
        return result;
    }

    public async Task<HttpResponseMessage> CreateExperienceAsync(CreateExperienceDetailCommand command)
    {
        var response = await identityHttpClient.PostAsJsonAsync("Profile/СreateExperienceDetail", command);
        return response;
    }
    ...
    {
        var response = await identityHttpClient.GetFromJsonAsync<SmsVerificationCodeStatus>($"sms/verify_code?PhoneNumber={phoneNumber}&Code={code}");
        return response;
    }

    public async Task<List<UserEducationResponse>> GetAllEducations()
    {
        var result = await identityHttpClient.GetFromJsonAsync<List<UserEducationResponse>>("Profile/GetAllEducations");
        return result;
    }

    public async Task<List<UserExperienceResponse>> GetAllExperiences()
    {
        var result = await identityHttpClient.GetFromJsonAsync<List<UserExperienceResponse>>("Profile/GetAllExperiences");
        return result;
    }

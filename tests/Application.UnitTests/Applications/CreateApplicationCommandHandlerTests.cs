...
public class CreateApplicationCommandHandlerTests : BaseTestFixture
{
    ...
    public async Task Handle_ValidRequest_ShouldCreateApplicationAndTimelineEvent()
    {
        ...
        CreateApplicationCommand request = new CreateApplicationCommand
        {
            ...
            CV = "https://www.indeed.com/career-advice/cover-letter-samples/devops-engineer",
            VacancyId = Guid.NewGuid()
        };

        Domain.Entities.Applicant applicant = new Domain.Entities.Applicant { Id = request.ApplicantId };
        _dbContextMock.Setup(x => x.Applicants.FindAsync(request.ApplicantId)).ReturnsAsync(applicant);

        JobVacancy vacancy = new JobVacancy { Id = request.VacancyId };
        _dbContextMock.Setup(x => x.Vacancies.FindAsync(request.VacancyId)).ReturnsAsync(vacancy);

        Mock<DbSet<ApplicationTimelineEvent>> timelineEventSetMock = new Mock<DbSet<ApplicationTimelineEvent>>();
        _dbContextMock.Setup(x => x.ApplicationTimelineEvents).Returns(timelineEventSetMock.Object);

        Mock<DbSet<Domain.Entities.Application>> applicationSetMock = new Mock<DbSet<Domain.Entities.Application>>();
        Guid newEntityGuid = Guid.NewGuid();
        applicationSetMock
            .Setup(d => d.AddAsync(It.IsAny<Domain.Entities.Application>(), It.IsAny<CancellationToken>()))
            .Callback<Domain.Entities.Application, CancellationToken>((v, ct) => v.Id = newEntityGuid);
